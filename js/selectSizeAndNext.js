    ...
    if (enabled) {
      ...
      if (!started) {
        ...
        if (size == null) {
          ...
          let addRemoveButtonsFirstElement =
            document.getElementById('add-remove-buttons').childNodes[1];

          if (
            toCleanString(addRemoveButtonsFirstElement.textContent) ==
            'sold out'
          ) {
          ...
          if (!colorFounded) {
            if (itemsCount != 1) {
              ...
              div.innerHTML =
                'Color not founded.' +
                '<br>' +
                'Select the color, the size and press ENTER';
