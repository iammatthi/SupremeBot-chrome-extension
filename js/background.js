...
isActive().then(function (response) {
  if (response) {
    chrome.runtime.onMessage.addListener(function (
      ...
      if (
        request ==
        'get enabled, started, counter, itemsCount, sites and itemsInBag'
      ) {
        sendResponse([
          enabled,
          started,
        ...
        request ==
        'get enabled, started, counter, sizes, itemsCount and colorFounded'
      ) {
        sendResponse([
          enabled,
          started,
        ...
        request == 'get enabled, started, delay, process payment reload'
      ) {
        ...
        request ==
        'get itemsCount, names, colors, types, sizes, delay and enabled'
      ) {
        sendResponse([
          names.length,
          names,
        ...
        isActive().then(function (response) {
          if (response) {
            ...
            if (
              ...
              (!request.save &&
                confirm(
                  'There are fields not filled in, do you want to save anyway?'
                ))
            ) {
        ...
        request.from === 'counter' &&
        request.subject === 'increment'
      ) {
        counter++;
      } else if (request.from === 'reset') {
        resetvars();
      } else if (
        request.from === 'itemsInBag' &&
        request.subject === 'increment'
      ) {
        itemsInBag++;
        ...
        request.from === 'autoRefresh' &&
        request.subject === 'confirm'
      ) {
...
function isActive() {
  let promise = new Promise(function (resolve, reject) {
    chrome.cookies.get(
      ...
      function (cookie) {
        if (cookie !== null && cookie.value >= new Date().getTime()) {
          if (enabled) {
          ...
          if (
            ...
            window.open(
              'mailto:matthi.berchtold@gmail.com?subject=SupremeBot renewal&body=I want to renewal my bot.'
            );
          }
...
function getTimeToString(time) {
  ...
  let pathMill = '000';
  return (
    path.substring(0, path.length - strHours.length) +
    strHours +
    ':' +
    path.substring(0, path.length - strMinutes.length) +
    strMinutes +
    ':' +
    path.substring(0, path.length - strSeconds.length) +
    strSeconds +
    '.' +
    pathMill.substring(0, pathMill.length - strMilliseconds.length) +
    strMilliseconds
  );
}
